...
#define PULSESYNC_PREFERRED_ROOT (1) // node with id==1 will become root if possible
#define PULSESYNC_BEACON_INTERVAL (5 * 1000 * 1000) // in ms
static void root_beacon_thread(void);
static void cyclic_driver_thread(void);
...
static uint32_t _pulsesync_beacon_interval = PULSESYNC_BEACON_INTERVAL;
static uint32_t _pulsesync_prop_time = 0;
static bool protocol_pause = true;

static uint16_t root_id = UINT16_MAX;
static uint16_t radio_id = 0;
        ...
        gtimer_timeval_t *toa)
{
    ...
    if (protocol_pause || radio_id == root_id || beacon->seq_number < seq_num)
    {
...
{
    _pulsesync_beacon_interval = delay_in_sec * 1000 * 1000;
}

void pulsesync_set_prop_time(uint32_t us)
{
    _pulsesync_prop_time = us;
}
...
{
    radio_id = get_transceiver_addr();
    if (radio_id == PULSESYNC_PREFERRED_ROOT)
    {
    ...
    if (beacon_thread_id == 0 && (root_id == radio_id))
    {
        beacon_thread_id = thread_create(beacon_stack,
        PULSESYNC_BEACON_STACK_SIZE,
        PRIORITY_MAIN - 2, CREATE_STACKTEST, root_beacon_thread,
                "pulsesync_beacon");
    }

    if (driver_thread_id == 0 && (root_id == radio_id))
    {
        driver_thread_id = thread_create(cyclic_stack,
        PULSESYNC_CYCLIC_STACK_SIZE,
        PRIORITY_MAIN - 2,
        CREATE_STACKTEST, cyclic_driver_thread, "pulsesync_cyclic_driver");
    }
...
{
    if ((num_entries >= PULSESYNC_ENTRY_VALID_LIMIT) || (root_id == radio_id))
        return PULSESYNC_OK;
    else
