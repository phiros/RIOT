...
#define BEACON_STACK_SIZE (KERNEL_CONF_STACKSIZE_MAIN)
#define CYCLIC_STACK_SIZE (KERNEL_CONF_STACKSIZE_MAIN)
...
char gtsp_beacon_buffer[SEND_BUFFER_SIZE] = { 0 };
...
static void _gtsp_beacon_thread(void) {
	while (1) {
		...
		puts("gtsp_beacon_thread unlocked\n");
	}
}
...
void gtsp_mac_read(uint8_t *frame_payload, uint16_t src, gtimer_timeval_t *toa) {
	...
	DEBUG("gtsp_read: Looking up src address: %" PRIu16 "\n", src);
